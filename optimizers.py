...
def create_train_op(loss, params):
    ...
    if "warmup_steps" in params.keys():
        lr = cosine_decay_with_warmup(tf.train.get_global_step(), lr, params["max_steps"], warmup_steps=opt_params["warmup_steps"])

    if opt_params["name"] == "adamW":
        optimizer = tf.contrib.opt.AdamWOptimizer(
            learning_rate=lr,
            weight_decay=lr*opt_params["weight_decay"],
        ...
        if opt_params["wd_type"] == "adam":
            ...
            decay_rate = adafactor_decay_rate_pow(opt_params["wd_exponent"])
        else:
